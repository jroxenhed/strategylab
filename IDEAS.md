...
- **Conviction-graded output, not buy/don't-buy (John, 2026-06-06: "we'll always be trading on a conviction scale").** Each ping carries a conviction number = signal strength (dose-score percentile) × playbook trust (charter CI width + live paper-tier forward record — earned and revocable) × portfolio context (correlation with held positions). Drives position size, Kelly-spirit: size proportional to edge. At ~$10k, granularity is a frozen 3-step ladder (small/half/full), finer as the account grows. The conviction formula is frozen and graded like any instrument — and because every ping is paper-tracked, conviction CALIBRATION is testable (did 80%-conviction setups actually beat 40%-conviction ones? flat curve = the conviction layer is decoration).
