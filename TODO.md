...
\*\*86 / 101 shipped.\*\* Themed roadmap. Items indexed **Section Letter + Number** (e.g. B3) for reference. Checked = done; journal has shipping details. Items below `### Pre-numbering` predate the addressing scheme.
...
- [ ] **B21a** Regime config not restored on page refresh — regime state loads from saved strategy but StrategyBuilder doesn't populate regime fields on mount. Needs load path in the strategy restore logic. [easy] [next]
- [ ] **A8c** "View as" 1D breaks on intraday backtests — candles switch from normal width to thin lines at a date boundary, x-axis labels mix monthly dates with intraday timestamps (15:30, 21). HTF indicator overlay also renders as smooth line instead of stepped when display interval matches HTF interval, and regime shading collapses to a thin bar. [medium] [next]
- [ ] **B23** Regime: dual rule sets — `long_buy_rules`/`long_sell_rules`/`short_buy_rules`/`short_sell_rules` in schema + backtest. Three-state regime (long/short/flat based on dual rule presence). Long/Short tab split in StrategyBuilder — each direction gets independent entry/exit rules with different indicators (e.g. MA-based long, RSI/Stochastic short). Prereq: B22. [medium] [next] [Plan](docs/superpowers/plans/2026-05-01-regime-filter.md)
