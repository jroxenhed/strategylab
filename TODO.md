...
- [ ] **F5** Group backend broker files into a `backend/brokers/` package. `broker.py` (680), `broker_aggregate.py`, `broker_health.py`, `broker_health_singleton.py` are a cohesive cluster at root. Moving them together preps for F2 (splitting `broker.py` by provider) without forcing it. Preserve `broker_health_singleton.py` as a separate module — it exists to break an import cycle, not as ceremony.
