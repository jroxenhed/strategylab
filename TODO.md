...
- [x] **D10** Compact mode for bot cards — toggle between current expanded layout and a condensed single-row view showing only essential info (heartbeat, symbol, strategy, P&L, status). Portfolio summary strip stays as-is. Persist preference to localStorage.
