...
- **Two-tier review architecture.** Builder and morning passes run in different environments and use different mechanisms — don't try to consolidate them.
  - **Overnight (builder, coverage role):** manual Task-tool dispatch is canonical. The `ce:review` skill does NOT resolve in the routine env (3 build-22 candidates failed; 11+ manual dispatches across builds 20-22 ran clean). Builder runs 4 always-on personas (correctness, testing, adversarial, security) + conditional personas based on diff (kieran-python/typescript, reliability, project-standards, maintainability). 4-6 per PR is the target, not 9 — duplicates inflate false confidence.
  - **Morning (calibration role):** ce:review skill works in the interactive session. 4-persona roster: adversarial (novel exploits), agent-native (architectural drift), security (uncovered adjacencies), reliability (severity judgment). Drop the dups the builder already covered. ~5 minutes: review, fix confirmed P1s, optional second-pass verification, merge.
