...
- **Subagent delegation rule.** Dispatch subagents when (a) wall-clock matters and work can run in parallel, (b) the subagent has a specialized capability (named persona, dedicated agent type), (c) the orchestrator is a higher model tier AND the dispatched work is substantially larger than the subagent's ~5-15k system-prompt write cost, or (d) intermediate work product is much larger than the final summary (multi-file scans, codebase tours, parallel adversarial reviews — noise stays in subagent context and dies with it). Don't dispatch when the only benefit would be "follow the rule" — inline work hits the parent's cache at 0.1× on the next turn, so single Read/Grep/Bash calls are cheaper inline than as a fresh haiku dispatch. The orchestrator always owns judgment (what to build, what to fix, what to defer) and verification (did the work happen as intended).
- **Subagent cache-write cost.** Each new subagent type dispatched starts a fresh API conversation: ~5-15k tokens written at 1.25× input rate. Same-type dispatches within ~3-5 min get a cache hit at 0.1× instead. Two implications: (1) parallel review of N personas pays N× the write cost vs sequential within the 5-min window — use parallel only when wall-clock matters (interactive sessions where you're waiting); overnight builder should sequence persona reviews within 5-min bursts when wall-clock doesn't matter. (2) Long-running subagents (>5 min) tick the parent's 5-min cache TTL without resetting it — a 10-min reviewer can force the parent to rewrite its cache on the next turn. Claude Code uses 1-hour TTL on stable prefixes (system prompt + skills) which mitigates this, but the conversation tail still ages out.
- **Orchestrator cycle (interactive sessions, Opus orchestrating Sonnet):**
  ...
  9. Repeat
- **Routine env (overnight builder).** Single Opus session executes end-to-end. **Reviews sequence within 5-min bursts**, not parallel — overnight has no wall-clock pressure, and N parallel persona dispatches pay N fresh 5-15k cache writes vs 1 write + N-1 hits when sequenced. Do sequential implementation directly (no tier arbitrage when both are Opus). Single-fixer pattern optional when fixes are mechanical, required when holistic. See `docs/overnight-builder-prompt-patch.md` for the procedural script.
...
- **Anti-patterns to avoid:**
  - Reflexive subagent dispatch for trivial reads — a single Read/Grep/Bash call lands in parent context at 0.1× cache cost next turn; a haiku dispatch pays a 5-15k write at 1.25× plus the summary round-trip. Inline wins for small work; subagent wins when the intermediate work is much larger than the summary (multi-file scans, codebase tours, parallel persona reviews).
